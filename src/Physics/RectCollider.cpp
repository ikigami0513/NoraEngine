...
glm::vec4 RectCollider::GetBounds() const {
    ...
    if (m_sprite && m_sprite->GetTexture()) { // Ajout d'une vérification sur la texture
        // Correction n°2 : Calculer la largeur/hauteur normalisée correctement
        ...
        // La taille réelle est la dimension de la texture * la proportion utilisée * l'échelle de l'objet
        ...
        // Le même problème d'origine existe ici si vous n'avez pas de sprite
        size = glm::vec3(scale.x, scale.y, 0.0f);
    }

    // Correction n°1 : Calculer min/max à partir du centre
